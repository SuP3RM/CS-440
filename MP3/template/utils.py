...
def configToIdx(config, offsets, granularity,alien):
    ...
    print("END 1 result",result)
    return tuple(result)

def idxToConfig(index, offsets, granularity,alien):
    # print("index,offsets,granularity,alien",index,offsets,granularity,alien)
    result = []
    for i in range(len(index[:2])):
        # print("index[i],granularity 2",index[i],granularity)
        result.append(int((index[i]*granularity)+offsets[i]))
        print("result 2",result)
        print(alien.get_shapes()[index[-1]])
    result.append(alien.get_shapes()[index[-1]])
    print("END 2 result",result)
    return tuple(result)
...
def noAlienidxToConfig(index,granularity,shape_dict):
    ...
    for i in range(len(index[:2])):
        ...
        print("no Alien id to config ",result)
    result.append(shape_dict[index[-1]])
    print("no Alien id to config END",result)
    return tuple(result)
